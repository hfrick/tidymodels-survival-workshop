...
Let's fit another PH and add some nonlinear terms via [splines](https://aml4td.org/chapters/interactions-nonlinear.html#sec-splines) and an [interaction](https://aml4td.org/chapters/interactions-nonlinear.html#sec-interactions):

```{r}
#| label: example-fit
# First add all of the predictors...
f <- event_time ~ . -
  # Them remove geocoded columns
  longitude - latitude +
...
```{r fit-resamples}
cat_res <- fit_resamples(cat_wflow, cat_folds, eval_time = 8:320)
cat_res
```
...
set.seed(2)
rf_res <- fit_resamples(rf_wflow, cat_folds, eval_time = 8:320, control = ctrl_cat)
...
final_fit <- last_fit(rf_wflow, cat_split, eval_time = 8:320) 
