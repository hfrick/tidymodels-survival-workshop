...
smooth_ph_linear_pred <- function(
  formula,
  data,
  deg_free = 5,
  grid_size = 500
  ...
  if (num_uniq <= deg_free) {
    cli::cli_abort(
      "The predictor has {num_uniq} unique values; more are \\ 
  ...
  pred_grid$linear_predictor <-
    predict(ph_fit, newdata = pred_grid, type = "lp", se.fit = FALSE) # TRUE appears to triggers a bug
