...
These are made-up cats. Each cat's predictors are randomly sampled values from the test set, so it is almost like a permutation-type analysis. 

Also, almost every model we consider will be strongly influenced by the `neutered` and `intake_type` columns, which have very few unique values. 

Our models will show _some_ signal (even though they theoretically should not) because the random shuffling might still keep the same values of these important predictors. 

Finally - for the weird formula assembly: this will be so much easier once we introduce recipes. 
...
We have to think a bit about how to compute the time point $t^*$. Let's say that we are making predictions about what will happen at $t^* = 17$ days. 
...
Second, what do we do when the evaluation time $\tau$ is less than the observed time $t_i$? We don't want to get the probability of being censored at some other time point, so we get the probability at $\tau$. 
...
\end{cases}
$$
How do we estimate $\hat{C}(t^*;\boldsymbol{x}_{i})$? We currently only estimate the probability of non-informative right censoring (i.e. the predictors $x_i$ are ignored). 

Our estimator $\hat{C}(T;x_i)$ is the “reverse Kaplan-Meier” (RKM, [Korn (1986)](https://scholar.google.com/scholar?hl=en&as_sdt=0%2C7&q=Korn+1986+%22Censoring+distributions+as+a+measure+of+follow-up+in+survival+analysis%22&btnG=)) curve that inverts the event indicator. This is the same for each sample at time $\tau$. 

We may expand this API in the future when you have informative censoring. 
...
How the censoring probability varies over time can have a big effect on the metrics. 

For the demonstration set, there are very few usable outcomes in the late stages of the analysis but these have large weights. 
