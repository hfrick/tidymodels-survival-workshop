...
fit(proportional_hazards(), event_time ~ 1, data = cat_train) %>% 
  ...
  labs(x = "Time", y = "Probability of remaining in the shelter") +
...
p_dt <- fit_dt %>% 
  ...
  mutate(intake_condition = factor(intake_condition, levels = c("ill_mild", "normal", "feral"))) %>% 
  tidyr::unnest(cols = .pred) %>% 
  ggplot() +
  geom_step(aes(.eval_time, .pred_survival, color = intake_condition)) +
  scale_color_discrete(
    labels = c("ill_mild, under_age_or_weight", "ill_moderatete, normal, other", "fractious, feral"),
...
## A model workflow `r hexes("censored", "parsnip", "workflows")`
...
#| code-line-numbers: "1-3|4-6|7|8-13"
#| fig.width: 8
#| fig.height: 6
#| fig.align: "center"
#| output-location: slide
predict(tree_fit, new_data = cat_test, type = "survival", 
  ...
  unnest(.pred) %>% 
  ggplot(aes(.eval_time, .pred_survival, 
             col = cat)) +
  geom_step() +
  labs(x = "Time", 
       y = "Probability of remaining in the shelter") +
  theme_bw(base_size = 18)
```
